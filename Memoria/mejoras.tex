...
\begin{lstlisting}[language=html,label=lst:confEvaluator,caption=Linea para crear una variable que indica que evaluador configurar]
   <script>var config_evaluator = "evaluator_follow_line.js";</script> 
\end{lstlisting}

\begin{lstlisting}[language=javascript,caption=Función que ejecuta el evaluador que se le ha pasado como variable en \textit{index.html}]
...
Para su evaluador en el que se crea una barra de progreso para cada robot y un cronómetro. Cuando se empiezan a mover los robots la barra de progreso empieza a completarse y el cronómetro se inicia, para comprobar el porcentaje completado se obtiene la posición del robot y la compara con el punto de llegada.
...
\end{lstlisting}
    \item Otra que crea un setInterval que comprueba la velocidad de los robots y, si es mayor que 0, se limpia el intervalo, se inician los evaluadores, llama a una función que realiza la funcionalidad para actualizar las barras de progreso y crea un cronómetro que añade al \textit{DOM}. Cronómetro y barras de progreso se actualizan cada 400 milisegundos mediante un \textit{setInterval}.
\begin{lstlisting}[language=javascript]
...
\end{lstlisting}
    \item Función que modifica las barras de progreso y muestra el porcentaje recorrido por cada uno de los robots comprobando su posición.
  \begin{lstlisting}[language=javascript]
...
var car={pos:{x:robot1.getPosition().x,z:robot1.getPosition().z},
    dist: 0
}
\end{lstlisting}

 \begin{lstlisting}[language=javascript,caption=Función que realiza la funcionalidad de la barra de progreso]
...
\begin{lstlisting}[language=javascript,caption={Código JavaScript que calcula la distancia entre \textit{drones}, la representa e incorpora un temporizador al \textit{DOM}}]
function setEvaluator(arrayRobots){
