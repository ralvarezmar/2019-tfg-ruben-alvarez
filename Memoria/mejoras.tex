...
En la tabla \ref{tab:tablaMotores2} se explican todas las funciones del \textit{HAL API} que extienden la plataforma para dar soporte a \textit{drones}.
...
Se han creado modelos de \textit{drones} de distintos colores para su disposición en ejercicios que requieran más de un \textit{robot} en el escenario: 
...
Además de los modelos de \textit{drones}, se han incluido nuevos \textit{robots} a \textit{WebSim}: 

\begin{itemize}
    \item \textbf{Fórmula 1}: se han creado dos modelos distintos para incorporarlos a ejercicios competitivos y que haya diferencias entre ambos para poder distinguirlos. 
 ...
 \label{fig:f1_renault}
   ...
   \label{fig:f1_williams}
        \end{subfigure}
          \caption{Modelos de fórmula 1}
    \end{figure}

    \item \textbf{mBot}: creado por la disponibilidad del modelo físico real y su facilidad para añadir elementos al \textit{robot} en la simulación.
    \begin{figure}[H]
    \centering            
...
Siendo el código fuente \textit{CSS} y \textit{HTML} empleado para el teleoperador del drone el mostrado a continuación: 
...
Estos teleoperadores tienen una arquitectura similar a las aplicaciones creadas de \textit{Scratch} o \textit{JavaScript}. A continuación se muestra un esquema con el diseño de esta aplicación:
